...
;   - 波浪号(~)：打开选中的网址或路径，如果路径是可执行程序则打开它,如果选中的是lnk快捷方式则打开该图标所在目录
;
...
; ===== 引用我的功能脚本 =====
#Include tools\tool.ahk   ; 调试用工具Ctrl+小键盘1,2,3,0。四个功能
...
#Include tools\CloseAdWindows.ahk   ; 关闭右下角弹窗，可以自己添加相关的类名即可
...
PauseScript() {
    ...
    if (ScriptPaused) {
        ...
        ;TrayTip("脚本已暂停")  ;右下角系统气泡提示（已改成暂停图标方式提醒）
    } else {
        try {
        ...
        ;TrayTip("脚本已恢复")  ;右下角系统气泡提示（已改成暂停图标方式提醒）
    }
}
